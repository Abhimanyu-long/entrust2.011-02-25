...
export const MyClients = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await getMyClients(125253); // Example client ID
        setMyClients(response);
      } catch (error) {
  ...
  // console.log("This is my clients ", myclients);
      ...
      <div className="card">
        <div className="card-header card-header-stretch border-bottom border-gray-200">
          <div className="card-title">
            <h3 className="fw-bold m-0">My Clients</h3>
          </div>
        </div>
